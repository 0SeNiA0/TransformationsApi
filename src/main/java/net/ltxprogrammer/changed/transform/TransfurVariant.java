...
public class TransfurVariant<T extends ChangedEntity> extends ForgeRegistryEntry<TransfurVariant<?>> {
    ...
    public IAbstractChangedEntity replaceEntity(@NotNull LivingEntity entity, @Nullable LivingEntity cause) {
        ...
        if (entity instanceof Player player) {
            ...
            if (!ProcessTransform.killPlayerByTransfur(player, cause != null ? cause : newEntity)) {
                newEntity.discard();
                var instance = ProcessTransform.setPlayerTransfurVariant(player, this, TransfurContext.hazard(TransfurCause.GRAB_REPLICATE), 1.0f);
                instance.willSurviveTransfur = true;

                ProcessTransform.forceNearbyToRetarget(player.level, player);

                ProcessTransform.onNewlyTransfurred(IAbstractChangedEntity.forPlayer(player));
    ...
    public static TransfurVariant<?> getEntityTransfur(LivingEntity entity) {
        return ProcessTransform.ifPlayerTransfurred(EntityUtil.playerOrNull(entity),
