...
import static net.ltxprogrammer.changed.transform.TransfurVariant.findEntityTransfurVariant;
    ...
    public <A extends AbstractAbilityInstance> A getAbilityInstance(AbstractAbility<A> ability) {
        try {
            if (this.underlyingPlayer != null) {
                return ProcessTransform.getPlayerTransfurVariant(this.underlyingPlayer).getAbilityInstance(ability);
            }
...
//TODO add new logic?
    public boolean isPreventingPlayerRest(Player player) {
        return false;// getLatexType().isHostileTo(LatexType.getEntityLatexType(player));
    }
    ...
    protected boolean targetSelectorTest(LivingEntity livingEntity) {
        ...
        if (livingEntity instanceof Player player) {
            if (!livingEntity.level.getGameRules().getBoolean(ChangedGameRules.RULE_NPC_WANT_FUSE_PLAYER))
                return false;
            var instance = ProcessTransform.getPlayerTransfurVariant(player);
    ...
    public boolean tryAbsorbTarget(LivingEntity target, IAbstractChangedEntity source, float amount, @Nullable List<TransfurVariant<?>> possibleMobFusions) {
        ...
        ProcessTransform.onAbsorbEntity(source);
        ...
        if (target instanceof Player loserPlayer) {
            if (!ProcessTransform.killPlayerByAbsorption(loserPlayer, source.getEntity())) { // Failed to kill player
                var instance = ProcessTransform.setPlayerTransfurVariant(loserPlayer, source.getTransfurVariant(), source.absorb(), 1.0f);
                instance.willSurviveTransfur = true;

                ProcessTransform.onNewlyTransfurred(IAbstractChangedEntity.forPlayer(loserPlayer));
            }
        }
    ...
    public boolean tryFuseWithTarget(LivingEntity entity, IAbstractChangedEntity source, float amount) {
        ...
        if (selfVariant != null && targetVariant != null) {
            ...
            { // Check if attacker can't fuse
                var instance = ProcessTransform.getPlayerTransfurVariant(underlyingPlayer);
            ...
            { // Check if attackee can't fuse
                var instance = ProcessTransform.getPlayerTransfurVariant(EntityUtil.playerOrNull(entity));
                if (instance != null && instance.ageAsVariant > level.getGameRules().getInt(ChangedGameRules.RULE_FUSABILITY_DURATION_PLAYER))
                    possibleFusion = List.of();
            }
            ...
            if (!possibleFusion.isEmpty()) {
                ...
                if (underlyingPlayer != null) {
                    if (entity instanceof Player pvpLoser) {
                        ProcessTransform.changeTransfur(underlyingPlayer, fusionVariant);
                        ChangedSounds.broadcastSound(underlyingPlayer, ChangedSounds.POISON, 1f, 1f);
                        ProcessTransform.killPlayerByAbsorption(pvpLoser, underlyingPlayer);
                    } else {
                        ProcessTransform.changeTransfur(underlyingPlayer, fusionVariant);
                        ChangedSounds.broadcastSound(underlyingPlayer, ChangedSounds.POISON, 1f, 1f);
                        entity.discard();
                    ...
                    ChangedSounds.broadcastSound(ProcessTransform.changeTransfur(entity, fusionVariant), ChangedSounds.POISON, 1f, 1f);
                    this.discard();
                }
        ...
        if (ProcessTransform.hasVariant(entity)) {
            if (!possibleMobFusions.isEmpty()) {
                ...
                if (underlyingPlayer != null) {
                    ...
                    ProcessTransform.setPlayerTransfurVariant(underlyingPlayer, mobFusionVariant, getAbsorbContext());
                    underlyingPlayer.setHealth(beforeHealth);
                }

                else if (entity instanceof Player victimPlayer) {
                    float beforeHealth = entity.getHealth();
                    ProcessTransform.setPlayerTransfurVariant(victimPlayer, mobFusionVariant, getAbsorbContext());
    ...
    public boolean tryTransfurTarget(Entity entity) {
        ...
        damage = ProcessTransform.difficultyAdjustTransfurAmount(entity.level.getDifficulty(), damage);
        TransfurVariant<?> variant = this.getTransfurVariant();

        if (entity instanceof LivingEntity livingEntity) {
            final var context = getReplicateContext();

            ProcessTransform.TransfurAttackEvent event = new ProcessTransform.TransfurAttackEvent(livingEntity, variant, context);
            if (Changed.postModEvent(event))
                return false;

            damage = ProcessTransform.checkBlocked(livingEntity, damage, context.source);
    ...
    protected void registerGoals() {
        ...
        if (this instanceof AbstractDarkLatexEntity)
